...
layui.define('view', function (exports) {
    ...
    , note: function (othis) {
      ...
      events.note.index = admin.popup({
        ...
        , success: function (layero, index) {
          ...
          // 打开即聚焦textarea，并绑定keyup事件，触发了则往本地数据表中存入{key:'note',value:this.value}数据
  ...
  admin.on('hash(side)', function (router) {
    // path的格式大致为：["component", "form", "element"]这样的数组结构
    var path = router.path, getData = function (item) {
        return {
          list: item.children('.layui-nav-child') //分组栏目的子栏目dl
          , name: item.data('name') //分组栏目li的data-name值
          , jump: item.data('jump') //分组栏目li的data-jump值
        }
      }
      , sideMenu = $('#' + SIDE_MENU)
      , SIDE_NAV_ITEMD = 'layui-nav-itemed'

      //捕获对应菜单(list参数是左侧菜单栏中所有li)
  ...
  //绑定所有定义了layadmin-event属性的标签的点击事件
  $body.on('click', '*[layadmin-event]', function () {
    var othis = $(this)
      , attrEvent = othis.attr('layadmin-event');
    // 寻找events全局变量，即admin.events对象上的对应方法，如果存在，则以当前点击的元素为上下文环境执行他
  ...
  //窗口resize事件(把resizeSystem方法挂载到layui.data对象上，layui.data是用于存储本地数据的接口，如：layui.data('test', {note: '本地记事本数据'})
