 ...
 * @param checked 是否选中
 * @param disabled 是否可选
 * @param iconCls 显示icon类名
 * @param children 子目录
 * @description 获取选中末节点id已改成获取全部选中id(含父级)
 */
...
layui.define(function (exports) {
  ...
  layuiXtree.prototype.Loading = function (options) {
    ...
    _this.xloading.style.marginTop = (_this._container.offsetHeight / 2 - 50) ? (_this._container.offsetHeight / 2 - 50) : 100 + 'px';
    ...
    // 修改：把请求方式由get改成post
    obj.open('post', options.data, true);
    ...
    // 修改：发送请求时候支持携带参数param,如果是对象，则转成json字符串对象形式
    options.param ? obj.send(JSON.stringify(options.param)) : obj.send();
  }
  ...
  layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
      for (i in d) {
        ...
        // 修改：把数据格式中的title改成name，value改成name
        _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + xtree_ischecked + xtree_isdisabled + ' value="' + d[i].id + '" title="' + d[i].name + '" lay-skin="primary" lay-filter="xtreeck' + _this._containerid + '">';
  ...
  // 更新渲染 // 修改：新增render传参渲染，将param与原有数据参数对象合并，如果同名，则此传入的优先
  layuiXtree.prototype.render = function (param) {
    var _this = this;
    // _this.Loading(_this._options);
    _this.Loading(Object.assign(_this._options, param));
  }
  ...
  //修改：获取全部选中的节点对象，而非仅仅末级checkbox对象
