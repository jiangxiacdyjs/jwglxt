    ...
    if(options.loading){
      if(hide){
        ...
        that.layInit = $(['<div class="layui-table-init">'
          ,'<i class="layui-icon layui-icon-loading layui-icon layui-anim-rotate layui-anim-loop"></i>'
        ,'</div>'].join(''));
      ...
      if (selectOnClick) {
        that.layBody.off('.selectOnClick').on('click.selectOnClick', 'tr', function (e) {
          var othis = $(this)
            ...
            , radioIpt = $(that.elem).find('input[lay-type="layTableRadio"]').eq(index)
            //复选模式下 对应显示checkbox
            ...
            , fixedRadioIpt = fixedTr.find('input[lay-type="layTableRadio"]').eq(0)
           ...
           */
          if (checkbox.length && !$(e.target).hasClass('layui-icon-ok') && checkbox.hasClass('layui-form-checked')) {
            // 设置隐藏的checkbox未选中
            ...
            // 仅仅只是起到同步全选状态效果而已
            that.syncCheckAll();
          } else if (checkbox.length && !$(e.target).hasClass('layui-icon-ok') && !checkbox.hasClass('layui-form-checked')) {
           ...
           * 如果当前点击对象不是显示选择图标所在处(有类名layui-icon-ok) 并且 当前点击行对应checkbox已显示选中状态，则手动设置“显示状态”及“实际状态”均为未选中状态
           */
          if (radio.length && !$(e.target).hasClass('layui-anim')) {
            var checked = radioIpt[0].checked
              ,thisData = table.cache[that.key]
              ,index = radioIpt.parents('tr').eq(0).data('index')
              ,ICON = ['&#xe643;', '&#xe63f;'];
            ...
            layui.each(thisData, function(i, item){
              if(index === i){
                // 默认模式下
                radio.addClass('layui-form-radioed').find('.layui-icon').addClass('layui-anim-scaleSpring').html(ICON[0]);
                oTr.find('input[lay-type="layTableRadio"]+').removeClass('layui-form-radioed').find('.layui-icon').removeClass('layui-anim-scaleSpring').html(ICON[1]);
                // fixed模式下
                var fixedDom = othis.closest(ELEM_MAIN).next(ELEM_FIXED);
                if(fixedDom.length){
                  var currentTr = fixedDom.find(ELEM_BODY).find('table').eq(0).find('tr').eq(index);
                  currentTr.siblings().find('input[lay-type="layTableRadio"]+').removeClass('layui-form-radioed').find('.layui-icon').removeClass('layui-anim-scaleSpring').html(ICON[1]);
                }
